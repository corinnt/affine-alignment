...
for j in range(1, len_n + 1):
    for i in range(1, len_m + 1):
        ...
        #todo: dp_table_best[][] registering as a tuple
...
#match_counter = 0
#for c in range(len(aligned_u)):
    #if (aligned_u[c] == aligned_v[c]):
        #match_counter += 1
...
#sys.stdout.write("percent identity: " + str(match_counter) + " / " + str(max(len(aligned_u), len(aligned_v))))
